...
| User PATH entry | `C:\Users\test-user\AppData\Local\Programs\Rust\cargo\bin` |
...
It printed `RUST_BUILD_SCRIPT_EXECUTION_OK` during the build and `RUST_EXECUTION_PROBE_OK` when the program ran. The output executable is `C:\Users\test-user\AppData\Local\Rust\target\debug\stay-up-rust-install-check.exe`.
